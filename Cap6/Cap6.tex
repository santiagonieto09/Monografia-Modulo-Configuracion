...
El sistema implementa una arquitectura orientada a eventos para desacoplar el microservicio \texttt{ms-debt-payments} del microservicio \texttt{ms-configuration}. El diagrama general de la Figura \ref{fig:observer} ilustra cómo se aplica el patrón Observer para la actualización asíncrona de contadores.
...
Para facilitar la adopción de estos estilos por parte del equipo de desarrollo y mantener la uniformidad en el código, se crearon definiciones globales en el archivo de estilos principal (styles.css). Aprovechando la arquitectura del framework Tailwind CSS, se
configuraron variables CSS personalizadas que mapean los colores institucionales a clases utilitarias.
Como se evidencia en la Figura \ref{fig:variablescss} se definieron las variables CSS para la paleta de colores; la imagen presenta una muestra representativa de los tonos configurados, existiendo definiciones adicionales para cubrir la totalidad de los estados y variacio
nes requeridas por la interfaz. Esto permite extender la configuración de Tailwind para utilizar clases semánticas como bg-primary o text-neutral-50, en lugar de códigos hexadecimales dispersos por el código.
...
Esta estrategia de implementación no solo asegura el cumplimiento de los lineamientos de diseño, sino que también optimiza el flujo de trabajo de los desarrolladores. La Figura \ref{fig:utilitarias} demuestra cómo se aplican estos estilos en el código HTML, combinando la sintaxis de utilidad de Tailwind con las definiciones personalizadas del sistema de diseño.
...
\end{figure}
 ...
 El resultado de esta abstracción es un código fuente más limpio y mantenible, donde la complejidad visual se encuentra encapsulada en las configuraciones del framework, garantizando que cualquier cambio futuro en la identidad visual pueda propagarse
 eficientemente a través de todo el sistema modificando únicamente las variables globales.