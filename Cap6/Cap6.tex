...
En este capítulo se presentan los aspectos técnicos y las decisiones de diseño durante la construcción del sistema, organizando la implementación en dos grandes áreas: el desarrollo del backend (lado del servidor) y la interfaz de usuario (frontend). La aplicación está desplegada en un entorno de pruebas, accesible mediante el siguiente enlace http://contables.unicauca.edu.co/dev
...
\subsection{Implementación del patrón publicador-suscriptor}

La implementación del patrón publicador-suscriptor en el módulo de Configuración se enfoca en la actualización asíncrona de contadores de uso de recursos, tales como Centros de Costo, en respuesta a eventos generados por otros microservicios, como \texttt{ms-debt-payments}. En este esquema, \texttt{ms-debt-payments} actúa como publicador, enviando notificaciones al utilizar un recurso. Estas notificaciones son consumidas por \texttt{ms-configuration}, que funciona como suscriptor, actualizando el contador correspondiente en la base de datos.

El sistema implementa una arquitectura orientada a eventos para desacoplar el microservicio \texttt{ms-debt-payments} del microservicio \texttt{ms-configuration}. El diagrama general de la \autoref{fig:observer} ilustra cómo se aplica el patrón publicador-suscriptor para la actualización asíncrona de contadores.

\begin{figure}[h!]
    \centering%
    \includegraphics[width=1.0\textwidth, height=1.0\textheight, keepaspectratio]{Cap6/Figuras/observer.png}    
    \caption{Diagrama de clases del patrón publicador-suscriptor}
    \label{fig:observer}
\end{figure}

\subsubsection{Motivación}

 La motivación para implementar el patrón publicador-suscriptor en este contexto radica en la necesidad de mantener actualizados los contadores de uso de recursos en el módulo de Configuración con el fin de validar las acciones de editar y eliminar si estos parametros han tenido movimientos contables lo cual evita inconsistencias en la generacion de reportes o procesos contables.
...
\begin{itemize}
    ...
    \item El microservicio \texttt{ms-configuration}, que contiene el submódulo de Centros de Costo, consume el evento a través del listener (\texttt{CostCenterUsageListener}).
...
\begin{itemize}
\item \textbf{Switch de tipo:} Evalúa \verb|event.getType()|, actualmente soporta \verb|EventUsageType.USED|.
\item \textbf{Validación:} Llama a \verb|isValidEvent()| para asegurar que \verb|costCenterId| no sea nulo y que \verb|quantityUsed| sea mayor que cero.
\item \textbf{Acción:} Si es válido, invoca a \verb|incrementUsageCount(data.getCostCenterId())|.
\item \textbf{Manejo de Errores:} Si el evento no es válido o el tipo no es soportado, lanza excepciones como \verb|IllegalArgumentException|.
\end{itemize}
...
\newpage
Esta metodología de desarrollo no solo permite la adherencia a las directrices de diseño, sino que además mejora la eficiencia en el proceso de trabajo de los programadores. La Figura \ref{fig:utilitarias} ilustra la manera en que se integran estos estilos en el código HTML, fusionando la sintaxis utilitaria de Tailwind con las configuraciones específicas del sistema de diseño.
