\begin{longtable}{|p{1.8cm}|p{4.5cm}|p{2.5cm}|p{5.5cm}|}
\caption{Historias épicas}
\label{tab:epicas} \\
\hline
\multicolumn{2}{|c|}{\textbf{Épica}} & \multicolumn{2}{c|}{\textbf{Historia de usuario asociada}} \\
\hline
\endfirsthead
\multicolumn{4}{c}{{\bfseries Tabla \thetable\ Continuación: Historias épicas}} \\
\hline
\multicolumn{2}{|c|}{\textbf{Épica}} & \multicolumn{2}{c|}{\textbf{Historia de usuario asociada}} \\
\hline
\endhead
\hline
\endfoot
\endlastfoot
\textbf{ID Épica} & \textbf{Nombre Épica} & \textbf{ID Historia} & \textbf{Nombre Historia} \\
\hline
\multirow{7}{1.8cm}{HE-01} & \multirow{7}{4.5cm}{Gestionar el catálogo de cuentas} & HE-01-HU01 & Crear cuentas contables \\
\cline{3-4}
 &  & HE-01-HU02 & Modificar cuentas contables \\
\cline{3-4}
 &  & HE-01-HU03 & Eliminar cuentas contables \\
\cline{3-4}
 &  & HE-01-HU04 & Importar catálogo de cuentas desde archivo plano \\
\cline{3-4}
 &  & HE-01-HU05 & Exportar catálogo de cuentas en un archivo plano \\
\cline{3-4}
 &  & HE-01-HU06 & Consultar cuentas contables \\
\cline{3-4}
 &  & HE-01-HU07 & Activar/Desactivar cuentas contables \\
\hline
\multirow{5}{1.8cm}{HE-02} & \multirow{5}{4.5cm}{Gestionar las tarifas de los impuestos} & HE-02-HU01 & Crear nuevas tarifas de impuestos \\
\cline{3-4}
 &  & HE-02-HU02 & Modificar tarifas de impuestos \\
\cline{3-4}
 &  & HE-02-HU03 & Eliminar tarifas de impuestos \\
\cline{3-4}
 &  & HE-02-HU04 & Consultar tarifas de impuestos \\
\cline{3-4}
 &  & HE-02-HU05 & Activar/Desactivar tarifas de impuestos \\
\hline
\multirow{7}{1.8cm}{HE-03} & \multirow{7}{4.5cm}{Gestionar la información de terceros} & HE-03-HU01 & Crear terceros \\
\cline{3-4}
 &  & HE-03-HU02 & Crear terceros desde PDF (RUT) \\
\cline{3-4}
 &  & HE-03-HU03 & Modificación de terceros \\
\cline{3-4}
 &  & HE-03-HU04 & Activar/Desactivar terceros \\
\cline{3-4}
 &  & HE-03-HU05 & Importar terceros desde archivo plano \\
\cline{3-4}
 &  & HE-03-HU06 & Exportar terceros en un archivo plano \\
\cline{3-4}
 &  & HE-03-HU07 & Consultar terceros \\
\hline
\multirow{6}{1.8cm}{HE-04} & \multirow{6}{4.5cm}{Gestionar el catálogo de productos del inventario} & HE-04-HU01 & Crear productos en el inventario \\
\cline{3-4}
 &  & HE-04-HU02 & Modificar productos en el inventario \\
\cline{3-4}
 &  & HE-04-HU03 & Eliminar productos en el inventario \\
\cline{3-4}
 &  & HE-04-HU04 & Importar productos desde archivo plano \\
\cline{3-4}
 &  & HE-04-HU05 & Exportar productos en un archivo plano \\
\cline{3-4}
 &  & HE-04-HU06 & Consultar productos en el inventario \\
\hline
\multirow{5}{1.8cm}{HE-05} & \multirow{5}{4.5cm}{Gestionar el centro de ayuda} & HE-05-HU01 & Crear centros de ayuda explicativos \\
\cline{3-4}
 &  & HE-05-HU02 & Modificar contenido en centros de ayuda \\
\cline{3-4}
 &  & HE-05-HU03 & Eliminar centros de ayuda \\
\cline{3-4}
 &  & HE-05-HU04 & Consultar centros de ayuda \\
\cline{3-4}
 &  & HE-05-HU05 & Activar/Desactivar centros de ayuda \\
\hline
\multirow{5}{1.8cm}{HE-06} & \multirow{5}{4.5cm}{Gestionar los métodos de pago} & HE-06-HU01 & Crear métodos de pago \\
\cline{3-4}
 &  & HE-06-HU02 & Modificar métodos de pago \\
\cline{3-4}
 &  & HE-06-HU03 & Eliminar métodos de pago \\
\cline{3-4}
 &  & HE-06-HU04 & Consultar métodos de pago \\
\cline{3-4}
 &  & HE-06-HU05 & Activar/Desactivar métodos de pago \\
\hline
\multirow{5}{1.8cm}{HE-07} & \multirow{5}{4.5cm}{Gestionar los tipos de documento} & HE-07-HU01 & Crear tipos de documento \\
\cline{3-4}
 &  & HE-07-HU02 & Modificar tipos de documento \\
\cline{3-4}
 &  & HE-07-HU03 & Eliminar tipos de documento \\
\cline{3-4}
 &  & HE-07-HU04 & Consultar tipos de documentos \\
\cline{3-4}
 &  & HE-07-HU05 & Activar/Desactivar tipos de documentos \\
\hline
\multirow{5}{1.8cm}{HE-08} & \multirow{5}{4.5cm}{Gestionar los bancos y sus cuentas bancarias} & HE-08-HU01 & Crear bancos \\
\cline{3-4}
 &  & HE-08-HU02 & Modificar bancos \\
\cline{3-4}
 &  & HE-08-HU03 & Eliminar bancos \\
\cline{3-4}
 &  & HE-08-HU04 & Consultar bancos \\
\cline{3-4}
 &  & HE-08-HU05 & Activar/Desactivar bancos \\
\hline
\multirow{6}{1.8cm}{HE-09} & \multirow{6}{4.5cm}{Gestionar los centros de costos} & HE-09-HU01 & Crear centros de costo \\
\cline{3-4}
 &  & HE-09-HU02 & Modificar centros de costo \\
\cline{3-4}
 &  & HE-09-HU03 & Eliminar centros de costo \\
\cline{3-4}
 &  & HE-09-HU04 & Consultar centros de costo \\
\cline{3-4}
 &  & HE-09-HU05 & Activar/Desactivar centros de costo \\
\cline{3-4}
 &  & HE-09-HU06 & Exportar Centros de Costo \\
\hline
\multirow{4}{1.8cm}{HE-10} & \multirow{4}{4.5cm}{Gestionar el calendario contable interactivo} & HE-10-HU01 & Abrir/cerrar año contable \\
\cline{3-4}
 &  & HE-10-HU02 & Abrir/cerrar mes contable \\
\cline{3-4}
 &  & HE-10-HU03 & Abrir/cerrar fecha específica \\
\cline{3-4}
 &  & HE-10-HU04 & Consultar periodos contables \\
\hline
\end{longtable}
